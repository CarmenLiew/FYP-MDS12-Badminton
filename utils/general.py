...
def write_pred_video(video_file, pred_dict, save_file, traj_len=8, label_df=None):
    ...
    # Manually set the codec (use 'XVID' or 'MJPG' if h264 doesn't work)
    fourcc = cv2.VideoWriter_fourcc(*'XVID')  # Try using 'XVID' for AVI format

    # Set the save file format to .avi for better compatibility
    save_file = save_file.replace('.mp4', '.avi')  # Ensure you're saving as .avi
